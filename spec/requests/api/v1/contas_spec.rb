...
RSpec.describe 'Contas API' do
  ...
  describe 'PUT /contas' do
    ...
    before do
      put "/contas/#{conta.id}", params: { conta: conta_params }.to_json, headers: headers
    end

    context 'Parâmetros válidos' do
      let(:conta_params){{ nome: 'Nova Conta' }}
      ...
      it 'Atualiza registro no banco de dados' do
        expect( Conta.find_by(nome: conta_params[:nome]) ).not_to be_nil
      end
    end
